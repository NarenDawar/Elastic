...
class Interpreter:
    ...
    def visit_FunctionNode(self, node, context):
        ...
        arg_names2 = [arg_name.value for arg_name in node.arg_names]
        func_value = Function(func_name, body_node, arg_names2, node.auto_return).set_context(context).set_position(node.pos_start, node.pos_end)
    ...
    def visit_CallNode(self, node, context):
        ...
        value_to_call = result.register(self.visit(node.call_node, context))
        if result.should_return(): return result
        ...
        return_val = result.register(value_to_call.execute(args))
        if result.should_return(): return result
